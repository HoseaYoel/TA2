...
# Read data 
data1 = read_excel("Proccessed2 Cambodia.xlsx", sheet = "mu (1) c")
data2 = read_excel("Proccessed2 Cambodia.xlsx", sheet = "lx c")
...
auto.arima(kt_ts) # 0,1,1

mod1 = Arima(kt_ts, order = c(0,1,1), include.drift = TRUE)
...
nmiu
write_xlsx(nmiu, "C:\\Users\\seraf\\Documents\\miu (1).xlsx")

#---------------------------------------------------------------------------------------
# mu (tau)
data3 = read_excel("Proccessed2 Cambodia.xlsx", sheet = "mu (tau) c")
data4 = read_excel("Proccessed2 Cambodia.xlsx", sheet = "lx c")
...
n_residue2 = -data3 - residue2 
lc2 = subset(n_residue2, select = -x)

# Heatmap
heatmap_data2 = as.matrix(lc2)
col_pal <- colorRampPalette(c("green", "yellow", "red"))(n = 1000)
...
auto.arima(kt_ts2) # 2,1,0

mod2 = Arima(kt_ts, order = c(2,1,0), include.drift = TRUE)
...
nmiu2
write_xlsx(nmiu2, "C:\\Users\\seraf\\Documents\\miu (tau).xlsx")

#-------------------------------------------------------------------
# New qx (tau)
qx_tau = 1 - exp(-nmiu2)
write_xlsx(qx_tau, "C:\\Users\\seraf\\Documents\\qx (tau).xlsx")
